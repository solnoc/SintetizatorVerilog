...
or_gate::or_gate(sf::Vector2f position, float angle, sf::Vector2f size) : binary_gate(position, angle, size)
{
	shape->setTexture(assets_class::get().getTexture("or_gate"));
	...
	{
		connective_lines[0].first = sf::Vertex(sf::Vector2f(GetInputPosition(0).position.x, GetInputPosition(0).position.y + size.x / 10.0f));
		connective_lines[1].first = sf::Vertex(sf::Vector2f(GetInputPosition(1).position.x, GetInputPosition(1).position.y + size.x / 10.0f));
	}
...
void or_gate::Draw(sf::RenderWindow* window)
{
